...
const CheckEventAttendanceModal = ({ onClose }) => {
  ...
  const updateAttendance = async (e) => {
    ...
    try {
      ...
      await makeRequest(
        'post',
        `/api/instructorapplication/EventAttendance/${eventsData[currentEvent].vendorReference}`,
      );

      setLoading(false);

      setSuccess(
        `Event Attendance processed for Event ${eventsData[currentEvent].vendorReference} – 
    ...
    <div className="w-screen">
      <div className="md-modal md-effect md-show border-2 border-white rounded-t-sm">
        <div className="inline-flex bg-primary text-white w-full items-center">
          <h2 className="flex-1 text-2xl p-2 font-agrandir">Check Event Attendance</h2>
        ...
        <div className="bg-white px-8 py-4">
          ...
          <div className="flex justify-center py-5">
            <span className="px-4">
              <ButtonWithIcon isLoading={loading} onClick={updateAttendance} title="Update Attendance">
...
export default CheckEventAttendanceModal;
